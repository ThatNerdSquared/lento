...
def test_start_block_controller_works_properly(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(_block_controller, "db", helpers.FakeSQLite())
    monkeypatch.setattr(Config, "SETTINGS_PATH", Path(tmp_path) / "lentosettings.json")
    monkeypatch.setattr(Config, "DAEMON_BINARY_PATH", Path("/tmp") / "lentodaemon")
    ...
    assert result == ["Windows daemon copied", "daemon launched"]
