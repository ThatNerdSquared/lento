...
def test_pf_blocks_websites_properly(monkeypatch, tmp_path):
    monkeypatch.setattr(
        Config,
        "SETTINGS_PATH",
    ...
    Config.SETTINGS_PATH.write_text(
        json.dumps(helpers.data["bare_config_with_all_blocked_sites"])
    )

    fw = get_firewall()
    fw.block_websites('Untitled Card')
...
def test_pf_unblock_websites_completely(monkeypatch, tmp_path):
    monkeypatch.setattr(
        Config,
        "PF_ANCHOR_PATH",
        Path(tmp_path) / "io.github.lento"
