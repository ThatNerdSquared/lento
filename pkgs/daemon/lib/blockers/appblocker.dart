    ...
    for (var rawline in processes) {
      ...
      if (blockedApps.containsKey(procName)) {
        ...
        if (!app['isSoftBlock']) {
          ...
          await Process.run(notifHelperPath, ['popup', '$procName hard-blocked', 'Lento has hard-blocked the app "$procName" during your work session. Your message: $popupMessage']);
        } else {
          if (DateTime.now().difference(app['lastOpened']).inMinutes > 15) {
            var pid = trimmedLine.substring(0, firstSpace - 1);
            await Process.run('kill', [pid]);
            String isAllowedString = (await Process.run(notifHelperPath, ['popup', '$procName soft-blocked', 'Lento has soft-blocked the app "$procName" during your work session. Does usage need to be extended by 15 minutes?'])).stdout();
            var isAllowed = isAllowedString == 'flutter: AlertButton.yesButton' ? true: false;
            if (isAllowed) {
